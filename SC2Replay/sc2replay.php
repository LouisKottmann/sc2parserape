...
class SC2Replay {
	...
	function parseInitDataFile($string) {
		...
		while (true) {
			...
			$depHash = $depHash[1];
			$str = "Unknown";
			if (class_exists("SC2ReplayUtils") && isset(SC2ReplayUtils::$depHashes[$depHash])) {
    ...
    private function parse0x01XBEvent(&$string, $playerId, $time, &$numByte, &$events) {
            if ($this->build >= 16561) {
        ...
        private function parse0x01ACEvent(&$string, $time, $playerId, $eventCode, &$numByte, &$nextByte) {
            if ($this->build >= 16561) {
	...
	private function parseGameEventsFile(&$string) {
		...
		while ($numByte < $len) {
				...
				case 0x01: // action
					switch ($eventCode) {
						...
						case 0x0B: // player uses an ability
							$this->parse0x01XBEvent($string, $playerId, $time, $numByte, $events);
                            break;
						...
						case 0xAC: // player changes selection
							$this->parse0x01ACEvent($string, $time, $playerId, $eventCode, $numByte, $nextByte);
                            break;
						...
						case 0x9D:
							...
							if ($this->build >= 16561) { 
								...
								//if ($byte1 == 0x0A || $byte1 == 0x09) {
                                    $tempStorage = (MPQFile::readByte($string,$numByte)) & 0x0F;
									$numByte += $tempStorage;
									break;
								}
								$extraBytes = floor($byte1 / 8);
            ...
            //$this->debug(sprintf("event: Timestamp: 
		}
